...
fn test_env() -> Option<TestEnv> {
    let mcp_base_url = match std::env::var("CLOTHO_AGENT_TEST_MCP_URL") {
        Ok(url) if !url.is_empty() => url,
        _ if env_truthy("CLOTHO_TEST_FAIL_ON_SKIP") => {
            panic!(
                "live-test gate refused to skip: CLOTHO_AGENT_TEST_MCP_URL not set (start the stack via `just dev`)"
            )
        }
        _ => {
            eprintln!(
                "skipping: CLOTHO_AGENT_TEST_MCP_URL not set (start the stack via `just dev`)"
            );
            return None;
        }
    };
...
async fn revoke_fixture_tokens(env: &TestEnv, agent_name: &str) -> Result<(), String> {
    if !agent_name.starts_with("weaver-") && !agent_name.starts_with("outsider-") {
        return Err(format!(
            "refusing to revoke tokens for non-test agent {agent_name:?}"
        ));
    }
    ...
    let response = http
        ...
        .send()
        .await
        .map_err(|error| format!("list fixture tokens for {agent_name}: {error}"))?;
    ...
    let body = response
        .text()
        .await
        .map_err(|error| format!("read fixture token list for {agent_name}: {error}"))?;
    if !status.is_success() {
        return Err(format!(
            "list fixture tokens for {agent_name}: {status}: {body}"
        ));
    }
    let tokens: Value = serde_json::from_str(&body)
        .map_err(|error| format!("parse fixture token list for {agent_name}: {error}"))?;
    for token in tokens["tokens"]
        .as_array()
        .ok_or_else(|| format!("fixture token list missing array for {agent_name}"))?
    {
        ...
        let token_id = token["id"]
            .as_str()
            .ok_or_else(|| format!("fixture token metadata missing id for {agent_name}"))?;
        let response = http
            .delete(format!(
            ...
            .send()
            .await
            .map_err(|error| format!("revoke fixture token {token_id}: {error}"))?;
        if response.status() != reqwest::StatusCode::NO_CONTENT
            && response.status() != reqwest::StatusCode::NOT_FOUND
        {
            return Err(format!(
                "revoke fixture token {token_id}: {}: {}",
                response.status(),
...
async fn cleanup_repo_fixture(env: &TestEnv, repo: &str, failed: bool) -> Result<(), String> {
    if !repo.starts_with("stage4-") {
        return Err(format!("refusing to clean non-test repository {repo:?}"));
    }
    ...
    Err(format!(
        "delete agent-test repository {repo}: {}: {}",
        response.status(),
        response.text().await.unwrap_or_default()
    ))
}

fn combine_cleanup(results: impl IntoIterator<Item = Result<(), String>>) -> Result<(), String> {
...
async fn scoped_agent_checkpoints_breaks_and_restores_over_mcp() {
    ...
    // A unique repo per run, provisioned through the canonical REST product
    // edge and then seeded with a known-good commit.
    ...
    let outcome = std::panic::AssertUnwindSafe(async {
        ...
        let created: Value = created_response.json().await.unwrap();
        let initial_commit_id = created["initial_commit_id"].as_str().unwrap().to_string();
        let mut vcs = VcsClient::connect(env.vcs_grpc_url.clone()).await.unwrap();
        let good = vcs
            .commit(CommitRequest {
                repo: repo.clone(),
                parent_commit_ids: vec![initial_commit_id],
        ...
        // Retrying an accepted Action start through MCP replays the exact REST
        // resource instead of scheduling duplicate compute.
        let action_key = format!("mcp-action-{nanos}");
        ...
        assert!(
            err,
            "reused key with different intent succeeded: {conflict}"
        );
        assert_eq!(conflict["data"]["code"], "idempotency_conflict");
    ...
    // There is deliberately no agent-delete admin boundary. Revoke every
    // minted credential through the existing narrow endpoint, then clean the
    // unique repository through canonical REST. Agent/audit metadata remains
    // durable provenance until the product defines retention semantics.
    let primary_cleanup = revoke_fixture_tokens(&env, &agent_name).await;
    let outsider_cleanup = revoke_fixture_tokens(&env, &outsider_name).await;
    let repo_cleanup = cleanup_repo_fixture(&env, &repo, outcome.is_err()).await;
    let cleanup = combine_cleanup([primary_cleanup, outsider_cleanup, repo_cleanup]);
    match outcome {
